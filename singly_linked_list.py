...
class SinglyLinkedList:
    ...
    def pop(self):
        ...
        """
        if not self.head  or self.length ==0 : return "Linked list is empty fool!"
        if self.length == 1:
            self.head = None
            self.tail = None     
    ...
    def shift(self):
        ...
        """
        if not self.head : return "The list is empty"
        if self.length == 0: 
            self.head = None
            self.tail = None
    ...
    def insert(self, indx:int, value)-> bool:
        ...
        """
        if indx < 0 or indx > self.length: return False
        if indx == self.length: return self.push(value)
        if indx == 0 : return not not self.un_shift(value) # hahaha this is cool, double negate . i negate yor negation kaiba !!
    ...
    def delete(self, indx) -> Any :
        ...
        if indx < 0 or indx >= self.length : return None
        if indx == 0 : return self.shift()
        if indx == self.length -1 : return  self.pop() # it's the last element so just remove it 
